...
const initialDiscussionMessages = [
...
    const { setCurrentView } = useContext(MyContext);
    // NEW STATE: To hold the messages and the current input
    const [messages, setMessages] = useState(initialDiscussionMessages);
    const [inputMessage, setInputMessage] = useState('');

    // NEW FUNCTION: Handles message submission
    const handleSendMessage = (e) => {
        ...
        const newMessage = {
            ...
            time: timeString,
            align: "right", // Assume user is always sending from the right
        };
...
                                <Bar dataKey="uv" fill="#ffc658" name="Progress Score" activeBar={false} /> 
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                    
...
                        {messages.map((message) => ( // MODIFIED to use messages state
...
                        ))}
                    </div>
                    {/* MODIFIED: Added onSubmit handler and input value/onChange handlers */}
                    <form className="thread-input-form" onSubmit={handleSendMessage}>
                        <input 
                            type="text" 
